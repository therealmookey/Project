...
async function verwijderAdres(id) {
  ...
  // 🔥 Haal naam op voor logging
  const adres = alleAdressen.find(a => a.id === parseInt(id));
  const naam = adres?.instelling_naam || 'Onbekend';
  ...
  try {
    ...
    if (error) {
      ...
      // 🔥 LOG: Adres verwijderd
      await logActie('verwijderd', 'adressen', id, naam);
...
async function saveAddress() {
  ...
  try {
    let result;
    const isBewerken = !!currentAddressId;
    ...
    if (result.error) {
      ...
      // 🔥 LOG: Adres toegevoegd of bijgewerkt
      const actie = isBewerken ? 'bijgewerkt' : 'toegevoegd';
      const entityId = isBewerken ? currentAddressId : result.data?.[0]?.id;
      await logActie(actie, 'adressen', entityId, adresData.instelling_naam);
